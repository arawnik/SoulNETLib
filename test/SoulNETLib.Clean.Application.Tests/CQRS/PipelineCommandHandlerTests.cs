using SoulNETLib.Clean.Application.Abstractions.CQRS;
using SoulNETLib.Clean.Domain;

namespace SoulNETLib.Clean.Application.Tests.CQRS;

/// <summary>
/// Tests for <see cref="PipelineCommandHandler{TCommand}"/> (non-generic Result variant).
/// </summary>
public sealed class PipelineCommandHandlerTests
{
    [Fact]
    public async Task Handle_NoBehaviors_CallsInnerHandlerDirectly()
    {
        var handler = new TestCommandHandler();
        var pipeline = new PipelineCommandHandler<TestCommand>(handler, []);

        var result = await pipeline.Handle(
            new TestCommand("test"),
            TestContext.Current.CancellationToken
        );

        Assert.True(result.IsSuccess);
        Assert.Equal(1, handler.CallCount);
    }

    [Fact]
    public async Task Handle_SingleBehavior_ExecutesAroundHandler()
    {
        var handler = new TestCommandHandler();
        var log = new List<string>();
        var behavior = new PassThroughBehavior(log);
        var pipeline = new PipelineCommandHandler<TestCommand>(handler, [behavior]);

        var result = await pipeline.Handle(
            new TestCommand("test"),
            TestContext.Current.CancellationToken
        );

        Assert.True(result.IsSuccess);
        Assert.Equal(1, handler.CallCount);
        Assert.Equal(["before", "after"], log);
    }

    [Fact]
    public async Task Handle_BehaviorShortCircuits_DoesNotCallInnerHandler()
    {
        var handler = new TestCommandHandler();
        var behavior = new ShortCircuitBehavior();
        var pipeline = new PipelineCommandHandler<TestCommand>(handler, [behavior]);

        var result = await pipeline.Handle(
            new TestCommand("test"),
            TestContext.Current.CancellationToken
        );

        Assert.True(result.IsFailure);
        Assert.Equal("Blocked", result.Error!.Code);
        Assert.Equal(0, handler.CallCount);
    }

    [Fact]
    public async Task Handle_MultipleBehaviors_ExecuteInRegistrationOrder()
    {
        var handler = new TestCommandHandler();
        var log = new List<string>();

        var behaviors = new IPipelineBehavior<TestCommand>[]
        {
            new OrderedBehavior(log, "A"),
            new OrderedBehavior(log, "B"),
            new OrderedBehavior(log, "C"),
        };

        var pipeline = new PipelineCommandHandler<TestCommand>(handler, behaviors);
        await pipeline.Handle(new TestCommand("test"), TestContext.Current.CancellationToken);

        // First registered = outermost = executes first
        Assert.Equal(["A-before", "B-before", "C-before", "C-after", "B-after", "A-after"], log);
    }

    private sealed class PassThroughBehavior(List<string> log) : IPipelineBehavior<TestCommand>
    {
        public async Task<Result> Handle(
            TestCommand command,
            PipelineStep next,
            CancellationToken cancellationToken
        )
        {
            log.Add("before");
            var result = await next();
            log.Add("after");
            return result;
        }
    }

    private sealed class ShortCircuitBehavior : IPipelineBehavior<TestCommand>
    {
        public Task<Result> Handle(
            TestCommand command,
            PipelineStep next,
            CancellationToken cancellationToken
        )
        {
            return Task.FromResult(Result.Failure(new Error("Blocked", "Short-circuited")));
        }
    }

    private sealed class OrderedBehavior(List<string> log, string name)
        : IPipelineBehavior<TestCommand>
    {
        public async Task<Result> Handle(
            TestCommand command,
            PipelineStep next,
            CancellationToken cancellationToken
        )
        {
            log.Add($"{name}-before");
            var result = await next();
            log.Add($"{name}-after");
            return result;
        }
    }
}
