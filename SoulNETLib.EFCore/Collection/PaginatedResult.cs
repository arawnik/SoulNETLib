...
namespace SoulNETLib.EFCore.Collection
{
       ...
       where T : class
    {
        ...
        /// </summary>
        private readonly PaginatedList<T> _list;
        ...
        public static async Task<PaginatedResult<T>> CreateAsync(IQueryable<T> source, int currentPage, int pageSize)
        {
            return new PaginatedResult<T>(await PaginatedList<T>.CreateAsync(source, currentPage, pageSize));
        }
    }
}
